...
export default function Dashboard({ tenantSlug }: { tenantSlug: string }) {
  ...
  async function testAgent() { if (!selectedAgent) return; try { const session = await api<{ call_id: string; livekit_url: string; token: string }>(`agents/${selectedAgent.id}/test-session`, { method: "POST", body: JSON.stringify({ agent_id: selectedAgent.id, variables: {}, metadata: { source: "dashboard" } }) }); setNotice(`Sessão de teste criada: ${session.call_id}. Conectando em ${session.livekit_url || "LiveKit"}.`); await refresh(); } catch (error) { setNotice(String(error)); } }
  ...
  return <div className="shell">
    ...
    <main><header className="top"><div><div className="eyebrow">tenant / {tenantSlug}</div><h1>{sections.find(([id]) => id === section)?.[1]}</h1></div><div className="topActions"><button className="secondary" onClick={() => void refresh()}>Atualizar</button><button onClick={() => setSection("agents")}>Novo agente</button></div></header>
      ...
      {loading ? <div className="loading">Carregando operação…</div> : <>
        ...
        {section === "members" && <section className="card"><h2>Membros</h2><p className="muted">Gerencie owners, admins, developers, operators e viewers do workspace.</p><Empty>O gerenciamento de convites será habilitado quando o endpoint de memberships estiver disponível.</Empty></section>}
        {section === "settings" && <section className="split"><article className="card"><h2>Integrações</h2><p>Google Calendar e Gmail</p><button className="secondary" onClick={async () => { try { const result = await api<{ url: string }>("integrations/google/connect"); location.href = result.url; } catch (error) { setNotice(String(error)); } }}>Conectar Google</button></article><article className="card"><h2>API e segurança</h2><p className="muted">Segredos são criptografados com KMS e nunca retornam em texto puro.</p><div className="row static"><span><strong>API interna</strong><small>Proxy autenticado e isolado por tenant</small></span><span className="pill ok">ativa</span></div></article></section>}
      </>}
    </main>
  </div>;
}
