...
export async function createWorkspace(formData: FormData) {
  ...
  if (!name || !agentName || !voiceId)
    throw new Error("Empresa, agente e voz são obrigatórios");
  ...
  const slug = availableWorkspaceSlug(
    name,
    existing.rows.map((row) => row.slug),
  );
  ...
  try {
    ...
    await client.query(
      "INSERT INTO tenants(id,slug,name,status,settings) VALUES($1,$2,$3,'trial',$4::jsonb)",
      [
        tenantId,
        slug,
        name,
  ...
  try {
    ...
    const created = await fetch(`${apiUrl}/v1/agents`, {
      method: "POST",
      headers,
      body: JSON.stringify({ name: agentName, template_id: templateId }),
    });
    if (!created.ok)
      throw new Error(`Falha ao criar agente: HTTP ${created.status}`);
    const agent = (await created.json()) as { id: string };
    agentId = agent.id;
    const configured = await fetch(`${apiUrl}/v1/agents/${agent.id}/draft`, {
      method: "PATCH",
      headers,
      body: JSON.stringify({
        tts: {
    ...
    if (!configured.ok)
      throw new Error(`Falha ao configurar voz: HTTP ${configured.status}`);
  } catch (error) {
