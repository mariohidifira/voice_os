...
def main() -> int:
    ...
    external_pending = [
        ...
        {
            "area": "phase4",
            "item": "Real provider-backed WhatsApp audio latency evidence against <= 8 s p50",
            "current_gap": "provider_credentials_and_runtime_measurement",
        },
        {
            "area": "phase5",
            "item": "Externally reachable hosted /voiceos.js proof, custom-domain TLS, and host-site Lighthouse evidence",
            "current_gap": "external_deploy_and_host_validation",
        },
    ]
    recommended_next_external_steps = [
        ...
        "Provide provider credentials and measure real WhatsApp audio reply latency",
    ...
    if not phase5_locally_complete:
        ...
        recommended_next_external_steps.insert(
            3, "Run the widget build in an executor that can access G:\\ without Node EPERM"
        )

    summary = {
        "scope": "final_handoff_summary",
        "status_date": "2026-08-25",
        "project_completion_estimate_percent": 95 if phase5_locally_complete else 93,
        "local_proven": {
            "final_local_audit": {
            ...
            "phase4": {
                ...
                "evidence_summary_passed": phase4.get("local_acceptance", {}).get("passed"),
                "next_gap": phase4.get("next_gap"),
                "environment_blocker": phase4.get("local_acceptance", {}).get(
                    "environment_blocker"
                ),
            },
            "phase5": {
                ...
                "next_gap": phase5_hosted.get("next_gap"),
                "environment_blocker": phase5_hosted.get("environment_blocker"),
            },
        },
