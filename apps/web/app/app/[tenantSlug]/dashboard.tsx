...
export default function Dashboard({ tenantSlug }: { tenantSlug: string }) {
  ...
  async function saveAgent(event: FormEvent<HTMLFormElement>) {
    ...
    try { await Promise.all([api(`agents/${selectedAgent.id}/draft`, { method: "PATCH", body: JSON.stringify(body) }), api(`agents/${selectedAgent.id}/draft/tools`, { method: "PUT", body: JSON.stringify({ tool_ids: selectedToolIds }) })]); await openAgent(selectedAgent.id); setNotice("Rascunho e ferramentas salvos."); } catch (error) { setNotice(String(error)); }
  }
  ...
  async function openKnowledge(kb: Item) { setSelectedKnowledge(kb); setDocuments((await api<{ data: Document[] }>(`knowledge-bases/${kb.id}/documents`)).data); setQueryResults([]); }
  async function createKnowledge(event: FormEvent<HTMLFormElement>) { event.preventDefault(); const form = new FormData(event.currentTarget); try { const created = await api<Item>("knowledge-bases", { method: "POST", body: JSON.stringify({ name: form.get("name") }) }); event.currentTarget.reset(); await refresh(); await openKnowledge(created); setNotice("Base criada."); } catch (error) { setNotice(String(error)); } }
  async function ingestDocument(event: FormEvent<HTMLFormElement>) { event.preventDefault(); if (!selectedKnowledge) return; const form = new FormData(event.currentTarget); const file = form.get("file") as File; const text = String(form.get("text") ?? "").trim(); const url = String(form.get("url") ?? "").trim(); try { let response: Response; if (file?.size) { const upload = new FormData(); upload.set("file", file); response = await fetch(`/api/voiceos/knowledge-bases/${selectedKnowledge.id}/documents`, { method: "POST", body: upload }); } else { response = await fetch(`/api/voiceos/knowledge-bases/${selectedKnowledge.id}/documents`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ name: String(form.get("name") || url || "Documento"), text: text || undefined, url: url || undefined }) }); } if (!response.ok) throw new Error((await response.json().catch(() => ({})))?.detail?.message ?? `HTTP ${response.status}`); event.currentTarget.reset(); await openKnowledge(selectedKnowledge); setNotice("Documento recebido; a ingestão está sendo processada."); } catch (error) { setNotice(String(error)); } }
  async function deleteDocument(documentId: string) { if (!selectedKnowledge) return; try { await api(`knowledge-bases/${selectedKnowledge.id}/documents/${documentId}`, { method: "DELETE" }); await openKnowledge(selectedKnowledge); setNotice("Documento removido."); } catch (error) { setNotice(String(error)); } }
  async function queryKnowledge(event: FormEvent<HTMLFormElement>) { event.preventDefault(); if (!selectedKnowledge) return; const form = new FormData(event.currentTarget); try { const result = await api<{ data: Array<Record<string, unknown>> }>(`knowledge-bases/${selectedKnowledge.id}/query`, { method: "POST", body: JSON.stringify({ query: form.get("query"), top_k: 5, min_score: 0.65 }) }); setQueryResults(result.data); setNotice(result.data.length ? "Busca vetorial concluída." : "Nenhum trecho atingiu o score mínimo."); } catch (error) { setNotice(String(error)); } }
  ...
  async function testTool(tool: Item) { try { const result = await api<Record<string, unknown>>(`tools/${tool.id}/test`, { method: "POST", body: JSON.stringify({ arguments: {}, session_variables: {}, end_user: {} }) }); setToolTestResult(JSON.stringify(result, null, 2)); await refresh(); setNotice(`Teste de ${tool.name} concluído.`); } catch (error) { setToolTestResult(String(error)); setNotice(String(error)); } }
  ...
  return <div className="shell">
    ...
    <main><header className="top"><div><div className="eyebrow">tenant / {tenantSlug}</div><h1>{sections.find(([id]) => id === section)?.[1]}</h1></div><div className="topActions"><button className="secondary" onClick={() => void refresh()}>Atualizar</button><button onClick={() => setSection("agents")}>Novo agente</button></div></header>
      ...
      {loading ? <div className="loading">Carregando operação…</div> : <>
        ...
        {section === "knowledge" && <section className="workspace"><aside className="list card"><h2>Bases de conhecimento</h2><form className="inline" onSubmit={createKnowledge}><input name="name" placeholder="Nome da base" required/><button>+</button></form>{knowledge.map((kb) => <button className={`row ${selectedKnowledge?.id === kb.id ? "selected" : ""}`} key={kb.id} onClick={() => void openKnowledge(kb)}><span><strong>{kb.name}</strong><small>{String(kb.embedding_model ?? "text-embedding-3-small")}</small></span><span className="pill ok">{String(kb.status ?? "ativa")}</span></button>)}</aside><article className="card editor">{selectedKnowledge ? <><div className="eyebrow">base de conhecimento</div><h2>{selectedKnowledge.name}</h2><form className="formGrid ingest" onSubmit={ingestDocument}><div className="two"><Field label="Arquivo PDF, DOCX ou HTML"><input name="file" type="file" accept=".pdf,.docx,.html,.htm,.txt"/></Field><Field label="URL"><input name="url" type="url" placeholder="https://…"/></Field></div><Field label="Nome (texto/URL)"><input name="name" placeholder="FAQ comercial"/></Field><Field label="Ou cole o conteúdo"><textarea name="text" rows={4}/></Field><button className="save">Adicionar documento</button></form><h2>Documentos</h2>{documents.map((document) => <div className="row static" key={document.id}><span><strong>{document.name}</strong><small>{document.source_type} · {document.chunk_count ?? 0} chunks{document.error ? ` · ${document.error}` : ""}</small></span><span><span className={`pill ${document.status === "ready" ? "ok" : ""}`}>{document.status}</span> <button className="iconDanger" onClick={() => void deleteDocument(document.id)}>×</button></span></div>)}{!documents.length && <Empty>Nenhum documento nesta base.</Empty>}<h2>Consultar base</h2><form className="inline" onSubmit={queryKnowledge}><input name="query" placeholder="Faça uma pergunta para validar os chunks" required/><button>Buscar</button></form>{queryResults.map((result, index) => <div className="queryResult" key={String(result.id ?? index)}><b>score {Number(result.score ?? 0).toFixed(3)}</b><p>{String(result.content ?? "")}</p></div>)}</> : <Empty>Selecione ou crie uma base para adicionar documentos e testar a recuperação.</Empty>}</article></section>}
